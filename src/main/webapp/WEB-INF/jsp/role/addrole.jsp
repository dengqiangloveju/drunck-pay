	...
	<form action="" method="post" class="form form-horizontal" id="form-role-add">
		<div class="row cl">
			...
			<div class="formControls col-10">
				<input type="text" class="input-text" value="" placeholder="" id="name" name="name" datatype="*2-30" nullmsg="角色名称不能为空">
			</div>
		</div>
		<div class="row cl">
				...
				<c:if test="${!empty privilages}">
					<c:forEach items="${privilages}" var="privilage">
						<dl class="permission-list">
							<dt class="m">
								<label><input type="checkbox" name="pids" value="${privilage.id}" name="user-Character-0" id="user-Character-0">${privilage.name}</label>
							</dt>
								...
								<c:forEach items="${privilage.children}" var="children">
									<dd>
										<dl class="cl permission-list2">
											<dt>
												<label><input type="checkbox" name="pids" value="${children.id}" name="user-Character-0-0" id="user-Character-0-0">${children.name}</label>
											</dt>
											<c:if test="${!empty children.children}">
												<dd>
													<c:forEach items="${children.children}" var="info">
														<label><input type="checkbox" name="pids" value="${info.id}" name="user-Character-0-0-0" id="user-Character-0-0-0">${info.name}</label>
													</c:forEach>
												</dd>
											</c:if>
										</dl>
...
$(function(){
	...
	$("#submit").on("click", function() {
		$.ajax({ 
		    type:'post',   
		    url:'${pageContext.request.contextPath}/role/createRole', 
